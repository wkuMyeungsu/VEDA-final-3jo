# run_operator.ps1 - 관제 센터 원클릭 실행 스크립트
param(
    [switch]$BuildOnly,
    [switch]$NoDemo
)
...
$QtDir = Join-Path $RootDir "operator-device\qt"
$ExePath = Join-Path $QtDir "build\windows-mingw\control_center.exe"
...
if (!(Test-Path $ExePath) -or $BuildOnly) {
    Write-Host "[INFO] control_center 빌드를 시작합니다..." -ForegroundColor Cyan
    Push-Location $QtDir
    try {
...
Write-Host "[INFO] 관제 센터를 실행합니다 ($ArgsList)..." -ForegroundColor Green
Write-Host "[INFO] 로그인 정보 -> ID: hanwha / PIN: 5hanwha!" -ForegroundColor Yellow
