...
// forklift-device/qt/common/network/RiskEventSource에서 이식 (동일 구현, 프로토콜 공용)
...
private:
    ...
    // [주의] 지금은 데모가 지게차 1대 기준이라 카메라 1개로 충분하지만, 여러
    // 대를 동시에 관제하게 되면 마지막 카메라 하나만 NetworkDisconnected로
    // 표시되는 한계가 있음 - 그때는 본 시점까지 수신된 전체 카메라 ID 집합을
    // 추적해서 전부에 통지하도록 확장 필요.
};
