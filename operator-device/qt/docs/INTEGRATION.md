# 실장비 연동 가이드 (operator-device/qt)

모든 통신(RTSP, MQTT) 모듈이 C++로 구현 완료되어 있으므로, **재빌드 없이 `config/*.json` 설정 수정만으로 실제 장비에 즉시 연결**됩니다.
...
## 2. 중앙 안전 서버 MQTT 연동 (`config/control_center.json`)
