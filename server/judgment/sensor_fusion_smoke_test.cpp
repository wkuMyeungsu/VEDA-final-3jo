...
// 쓰지 않는다 - 카메라 입력은 danger_judgment_engine_main.cpp의 더미 CameraInput
// 시나리오 패턴을 그대로 재사용한 고정값 하나뿐이다.
//
// 입력: 실제 I2C 버스(/dev/i2c-1)의 MPU6050(IMU) + 커널 캐릭터 디바이스(/dev/vl53l1x0)의
//       VL53L1X(ToF). 카메라 쪽은 더미 CameraInput 고정값.
// 처리: SensorCollectorReader::read()로 매 회 실제 센서값을 SensorInput으로 변환 ->
//       DangerJudgmentEngine::evaluate(cam, sen) 호출.
// 출력: 매 회 SensorInput 필드(imu_ok/tof_ok/tof_distance_m/imu_accel_g)와
...
#include "sensor_collector_reader.h"
...
int main() {
    ...
    // 더미 CameraInput: danger_judgment_engine_main.cpp 시나리오 1(정상-안전)과 동일한
    // 고정값. CameraInput 인자 순서: forklift_localized, person_detected, forklift,
    // person, camera_id, zone. 카메라 쪽은 이번 테스트 범위 밖이라 값을 바꾸지 않는다.
    const CameraInput cam{true, true, {0.0, 0.0}, {5.0, 5.0}, "", ""};
    ...
    for (int i = 0; i < kIterations; ++i) {
        ...
        std::cout << "[" << i << "] SensorInput: "
                  ...
                  << "tof_distance_m=" << std::fixed << std::setprecision(3) << sen.tof_distance_m << " "
