...
HOST = os.environ.get("SERVER_OPS_HOST", "0.0.0.0")
PORT = int(os.environ.get("SERVER_OPS_PORT", "8000"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            self.send_body(json.dumps({"ok": True, "service": "server-ops", "monitoring": "placeholder", "port": PORT}), "application/json")
            return
...
if __name__ == "__main__":
    print(f"server-ops listening on {HOST}:{PORT}")
