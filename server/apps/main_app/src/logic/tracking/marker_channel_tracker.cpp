...
MarkerStreamTracker::MarkerStreamTracker(int marker_id, int confirm_frames,
...
std::optional<std::string> MarkerStreamTracker::onArucoFrame(const ArucoFrame& frame,
    ...
    if (frame.stream_id.empty()) return std::nullopt;
    ...
    return update(frame.stream_id, marker_seen, now);
}

std::optional<std::string> MarkerStreamTracker::update(const std::string& stream_id,
                                                        bool marker_seen,
                                                        Clock::time_point now) {
    std::lock_guard<std::mutex> lk(mtx_);

    if (stream_id.empty()) return std::nullopt;
    StreamState& self = streams_[stream_id];
    if (marker_seen) {
        ++self.streak;
        self.last_seen = now;
        self.ever_seen = true;
    ...
    if (*active_ == stream_id) return std::nullopt;
    ...
    const StreamState& act = active_it->second;
...
std::optional<std::string> MarkerStreamTracker::activeStream() const {
    std::lock_guard<std::mutex> lk(mtx_);
    return active_;
}

int MarkerStreamTracker::streakOf(const std::string& stream_id) const {
    std::lock_guard<std::mutex> lk(mtx_);
    auto it = streams_.find(stream_id);
    return it == streams_.end() ? 0 : it->second.streak;
}

void MarkerStreamTracker::reset() {
    std::lock_guard<std::mutex> lk(mtx_);
    streams_.clear();
    active_.reset();
}
